...
// Ручное определение __dirname для поддержки ES-модулей
...
async function checkAndInitDatabase(forceRebuild = false) {
  try {
    ...
    if (forceRebuild) {
      console.log("⚠️ Запущен принудительный сброс и импорт структуры БД...");
...
async function startServer() {
  ...
  app.post('/api/analytics/simulate', (req, res) => {
    try {
      ...
      appHistoryFallback.length = 0; // Clear fallback arrays
      candidatesFallback.length = 0;
      ...
      const simulatePaths = (vacancyId: number, count: number, stagesCount: number, delayArr: number[]) => {
        for(let i=0; i<count; i++) {
          let curTime = baseDate - (Math.random() * 90 * dayMs);
      ...
      let multiplier = 1.0;
      if (dbType === 'fast') multiplier = 0.25;
      else if (dbType === 'strict') multiplier = 2.0;
      else if (dbType === 'ideal') multiplier = 0.5;

      const d1 = [1.2, 2.0, 7.8, 3.1, 2.5, 4.0, 1.5].map(x => x * multiplier);
      simulatePaths(1, 400, 1, d1);
      simulatePaths(1, 430, 2, d1);
      simulatePaths(1, 240, 3, d1);
      simulatePaths(1, 115, 4, d1);
      ...
      const d2 = [2.1, 3.2, 1.5, 4.0, 3.0, 2.0, 1.0].map(x => x * multiplier);
      simulatePaths(2, 300, 1, d2);
      simulatePaths(2, 250, 2, d2);
      simulatePaths(2, 180, 3, d2);
      simulatePaths(2, 90, 4, d2);
      ...
      const d3 = [1.0, 1.5, 2.2, 8.5, 1.8, 1.0, 2.0].map(x => x * multiplier);
      simulatePaths(3, 200, 1, d3);
      simulatePaths(3, 170, 2, d3);
      simulatePaths(3, 110, 3, d3);
      simulatePaths(3, 50, 4, d3);
      ...
      const d4 = [2.5, 4.0, 3.5, 6.0, 2.0, 3.0, 1.8].map(x => x * multiplier);
      simulatePaths(4, 150, 1, d4);
      simulatePaths(4, 120, 2, d4);
      simulatePaths(4, 80, 3, d4);
      simulatePaths(4, 45, 4, d4);
      ...
      const d5 = [3.0, 2.5, 5.0, 2.0, 4.5, 1.5, 2.0].map(x => x * multiplier);
      simulatePaths(5, 100, 1, d5);
      simulatePaths(5, 80, 2, d5);
      simulatePaths(5, 60, 3, d5);
      simulatePaths(5, 30, 4, d5);
      ...
      const d6 = [1.5, 2.0, 3.0, 1.2, 5.5, 2.5, 1.0].map(x => x * multiplier);
      simulatePaths(6, 110, 1, d6);
      simulatePaths(6, 95, 2, d6);
      simulatePaths(6, 70, 3, d6);
      simulatePaths(6, 40, 4, d6);
      ...
      const d7 = [4.5, 1.5, 1.2, 2.0, 1.5, 2.0, 1.0].map(x => x * multiplier);
      simulatePaths(7, 330, 1, d7);
      simulatePaths(7, 300, 2, d7);
      simulatePaths(7, 120, 3, d7);
      simulatePaths(7, 58, 4, d7);
      ...
      const d8 = [1.0, 2.2, 1.8, 1.5, 2.0, 1.0, 3.0].map(x => x * multiplier);
      simulatePaths(8, 90, 1, d8);
      simulatePaths(8, 80, 2, d8);
      simulatePaths(8, 50, 3, d8);
      simulatePaths(8, 30, 4, d8);
...
// ==========================================
// НАСТРОЙКА РАЗДАЧИ ФРОНТЕНДА ДЛЯ ДИПЛОМА
// ==========================================

// 1. Указываем Express раздавать статические файлы из папки "dist"
app.use(express.static(path.join(__dirname, 'dist')));

// 2. Для всех остальных GET-запросов возвращаем index.html (поддержка путей React)
app.get('*', (req, res) => {
